...
func GetUserGroup(ctx context.Context, token, gruppo, dominio, coreApiVersion string) (map[string]string, error) {
	...
	gruRes, _ := ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msusers", "/api/"+os.Getenv("API_VERSION")+"/users/GRU/equals(XGRU03,'"+gruppo+"')", token, dominio, coreApiVersion)
...
func GetNextVersion(ctx context.Context, masterBranch, nomeDocker, tenant, accessToken, loginApiDomain, coreApiVersion, resource, devopsToken string) (string, error) {
	...
	restyKubeImicroservRes, errImicroservRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsImicro, "msdevops", "/api/"+os.Getenv("API_VERSION")+"/devops/KUBEDKRBUILD", devopsToken, loginApiDomain, coreApiVersion)
...
func GetMicroserviceDetailDocument(ctx context.Context, team, ims, gitDevMaster, buildVersion, devopsToken, autopilot, enviro, dominio, coreApiVersion string) (Microservice, error) {
	...
	Logga(ctx, os.Getenv("JsonLog"), "Getting MICROSERVICE DOCUMENT")

	type MicroDocument struct {
	...
	var md MicroDocument
	md.Debug = false
	md.Dim = "KUBEIMICROSERV"
	md.Key = "XKUBEIMICROSERV03"
	md.ValKey = ims
	md.CenterDett = "allviews"

	relatedDimConditionArr := make(map[string][]string)
	relatedDimConditionArr["DEPLOYLOG"] = append(relatedDimConditionArr["DEPLOYLOG"], "XDEPLOYLOG06='1'")
	relatedDimConditionArr["KUBEDKRCONFIGMAP"] = append(relatedDimConditionArr["KUBEDKRCONFIGMAP"], "XKUBEDKRCONFIGMAP04='"+enviro+"'")

	md.RelatedDimCondition = relatedDimConditionArr

	relatedDimArr := make(map[string][]string)
	relatedDimArr["DEPLOYLOG"] = make([]string, 0)
	relatedDimArr["KUBEDKRCONFIGMAP"] = make([]string, 0)
	md.RelatedDim = relatedDimArr

	md.Compact = false
	md.UsePublicCode = false
	md.CenterDett = "allviews"
	md.Depth = 5
	md.DimOccurrences = 3
	...
	errMD := json.Unmarshal(resMd.Body(), &mdResponse)
	if errMD != nil {
		Logga(ctx, os.Getenv("JsonLog"), errMD.Error())
		return microservices, errMD
	}

	ms := mdResponse[0]["XKUBEIMICROSERV04"].(map[string]interface{})

	microservices.VersMicroservice, _ = ms["XKUBEMICROSERV07"].(string)
	microservices.Nome = ms["XKUBEMICROSERV05"].(string)
	microservices.Descrizione = ms["XKUBEMICROSERV03"].(string)
	microservices.Public = int(ms["XKUBEMICROSERV18"].(float64))
	microservices.Namespace = ms["XKUBEMICROSERV04_COD"].(string)
	microservices.Virtualservice = strconv.FormatFloat(ms["XKUBEMICROSERV08"].(float64), 'f', 0, 64)
	microservices.DatabasebEnable = strconv.FormatFloat(ms["XKUBEMICROSERV15"].(float64), 'f', 0, 64)

	// HPA
	msHpa := ms["XKUBEMICROSERV16"].(map[string]interface{})
	var hpa Hpa
	hpa.MinReplicas = strconv.FormatFloat(msHpa["XKUBEMICROSERVHPA04"].(float64), 'f', 0, 64)
	hpa.MaxReplicas = strconv.FormatFloat(msHpa["XKUBEMICROSERVHPA05"].(float64), 'f', 0, 64)
	hpa.CpuTipoTarget = msHpa["XKUBEMICROSERVHPA06"].(string)
	hpa.CpuTarget = strconv.FormatFloat(msHpa["XKUBEMICROSERVHPA07"].(float64), 'f', 0, 64)
	hpa.MemTipoTarget = msHpa["XKUBEMICROSERVHPA08"].(string)
	hpa.MemTarget = strconv.FormatFloat(msHpa["XKUBEMICROSERVHPA09"].(float64), 'f', 0, 64)
	microservices.Hpa = hpa

	// DKR
	msServDkr := ms["XKUBEMICROSERV14"].([]interface{})

	msServDkrMap := []interface{}{}
	b, _ := json.Marshal(msServDkr)
	errServDkr := json.Unmarshal(b, &msServDkrMap)
	if errServDkr != nil {
		Logga(ctx, os.Getenv("JsonLog"), errServDkr.Error())
		return microservices, errServDkr
	}

	for _, srvDkr := range msServDkrMap {

		srvDkrMap := srvDkr.(map[string]interface{})

		dkrMap := srvDkrMap["XKUBEMICROSERVDKR04"]
		dkrs := dkrMap.(map[string]interface{})

		LogJson(dkrs)

		pod.Docker = dkrs["XKUBEDKR03"].(string)
		pod.GitRepo = dkrs["XKUBEDKR04"].(string)

		// PODRESOURCE
		var resource Resource
		resDkr, ok := dkrs["XKUBEDKR05"].(map[string]interface{})
		if ok {
			resource.CpuReq = resDkr["XKUBEDKRRESOURCE04"].(string) //   -- cpu res
			resource.MemReq = resDkr["XKUBEDKRRESOURCE05"].(string) //   -- mem res
			resource.CpuLim = resDkr["XKUBEDKRRESOURCE06"].(string) //   -- cpu limit
			resource.MemLim = resDkr["XKUBEDKRRESOURCE07"].(string) //   -- mem limit
			pod.Resource = resource
		}

		pod.Descr = dkrs["XKUBEDKR08"].(string)
		pod.Dockerfile = dkrs["XKUBEDKR09_COD"].(string)

		resource.CpuReq = resDkr["XKUBEDKRRESOURCE04"].(string)

		pod.Tipo = dkrs["XKUBEDKR10"].(string)

		// MOUNT VECCHIA MANIERA
		msMou, ok := dkrs["XKUBEDKR11"].([]interface{})
		if ok {
			msMouMap := []map[string]interface{}{}
			b, _ := json.Marshal(msMou)
			errBui := json.Unmarshal(b, &msMouMap)
			if errBui != nil {
				Logga(ctx, os.Getenv("JsonLog"), errBui.Error())
				return microservices, errBui
			}

			var mounts []Mount
			for _, xMnt := range msMouMap {
				var mount Mount
		...
		// SERVICEDKR
		msSrv, ok := dkrs["XKUBEDKR12"].([]interface{})
		if ok {
			msSrvMap := []map[string]interface{}{}
			b, _ := json.Marshal(msSrv)
			errBui := json.Unmarshal(b, &msSrvMap)
			if errBui != nil {
				Logga(ctx, os.Getenv("JsonLog"), errBui.Error())
				return microservices, errBui
			}
			...
			for _, xSrv := range msSrvMap {

				var service Service
				portFloat, _ := xSrv["XKUBESERVICEDKR06"].(float64)
				port := strconv.FormatFloat(portFloat, 'f', 0, 64)

				tipo, _ := xSrv["XKUBESERVICEDKR05"].(string)
		...
		// VERSIONE DI BUILD O DEPLOYLOG
		if versione != "" && enviro == "int" { // --mondo DEPLOYLOG
			...
			msDpl, ok := mdResponse[0]["XKUBEIMICROSERV09"].([]interface{})
			if ok {
				msDplMap := []map[string]interface{}{}
				b, _ := json.Marshal(msDpl)
				errDpl := json.Unmarshal(b, &msDplMap)
				if errDpl != nil {
					Logga(ctx, os.Getenv("JsonLog"), errDpl.Error())
					return microservices, errDpl
				}

				tipoDeploy := msDplMap[0]["XDEPLOYLOG08"].(string)
		...
		} else { //-- mondo DOCKERBUILD

			msBui, ok := dkrs["XKUBEDKR14"].([]interface{})

			if ok {

				msBuiMap := []map[string]interface{}{}
				b, _ := json.Marshal(msBui)
				errBui := json.Unmarshal(b, &msBuiMap)
				if errBui != nil {
					Logga(ctx, os.Getenv("JsonLog"), errBui.Error())
					return microservices, errBui
				}
				...
				var branchs Branch
				branchs.Branch = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD04"].(string)
				branchs.Version = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD06"].(string)
				branchs.Sha = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD07"].(string)

				var podBuild PodBuild
				podBuild.Versione = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD06"].(string)
				podBuild.Merged = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD13"].(string)
				podBuild.Tag = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD09"].(string)
				podBuild.MasterDev = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD04"].(string)
				podBuild.ReleaseNote = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD12"].(string)
				podBuild.SprintBranch = msBuiMap[len(msBuiMap)-1]["XKUBEDKRBUILD10"].(string)
		...
		pod.Vpn = int(dkrs["XKUBEDKR15"].(float64))

		// CONFIGMAP
		var cfgMaps []ConfigMap
		msCon, ok := dkrs["XKUBEDKR16"].([]interface{})
		if ok {
			msConMap := []map[string]interface{}{}
			b, _ := json.Marshal(msCon)
			errBui := json.Unmarshal(b, &msConMap)
			if errBui != nil {
				Logga(ctx, os.Getenv("JsonLog"), errBui.Error())
				return microservices, errBui
			}

			for _, xCfg := range msConMap {
		...
		// PROBE
		var probes []Probes
		msPro, ok := dkrs["XKUBEDKR17"].([]interface{})
		if ok {
			msProMap := []map[string]interface{}{}
			b, _ := json.Marshal(msPro)
			errBui := json.Unmarshal(b, &msProMap)
			if errBui != nil {
				Logga(ctx, os.Getenv("JsonLog"), errBui.Error())
				return microservices, errBui
			}

			for _, xPrb := range msProMap {
		...
		// PODTMPL
		tmplDkr := dkrs["XKUBEDKR09"].(map[string]interface{})
		pod.Workdir = tmplDkr["XKUBEDKRTMPL07"].(string)
	...
	//LogJson(microservices)
	Logga(ctx, os.Getenv("JsonLog"), "Getting MICROSERVICE DOCUMENT OK")
	Logga(ctx, os.Getenv("JsonLog"), " - - - - - - - - - - - - - - -  ")
	Logga(ctx, os.Getenv("JsonLog"), "")
...
func GetMicroserviceDetail(ctx context.Context, team, ims, gitDevMaster, buildVersion, devopsToken, autopilot, enviro, dominio, coreApiVersion string) (Microservice, error) {
	...
	restyKubeImicroservRes, errKubeImicroservRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsImicro, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEIMICROSERV", devopsToken, dominio, coreApiVersion)
	...
	restyKubeMSRes, errKubeMSRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsMS, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEMICROSERV", devopsToken, dominio, coreApiVersion)
	if errKubeMSRes != nil {
	...
	if autopilot != "1" {
		...
		restyKubeHpaRes, errKubeHpaRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsHpa, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEMICROSERVHPA", devopsToken, dominio, coreApiVersion)
	...
	restyDkrLstRes, errDkrLstRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsDkr, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/devops/custom/SELKUBEDKRLIST/values", devopsToken, dominio, coreApiVersion)
	if errDkrLstRes != nil {
	...
	if len(restyDkrLstRes.BodyArray) > 0 {
		...
		for _, x := range restyDkrLstRes.BodyArray {
			...
			restyKubeMntRes, errKubeMntRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsMnt, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEDKRMOUNT", devopsToken, dominio, coreApiVersion)
			...
			restyKubeSrcRes, errKubeSrcRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsSrc, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEDKRRESOURCE", devopsToken, dominio, coreApiVersion)
			...
			restyKubePrbRes, errKubePrbRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsProbes, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEDKRPROBE", devopsToken, dominio, coreApiVersion)
			...
			restyKubeSrvDkrRes, errKubeSrvDkrRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsSrvDkr, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBESERVICEDKR", devopsToken, dominio, coreApiVersion)
...
func GetTenant(ctx context.Context, token, dominio, coreApiVersion string) ([]Tenant, error) {
	...
	tenantRes, errtenantRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msauth", "/api/"+os.Getenv("API_VERSION")+"/auth/tenants", token, dominio, coreApiVersion)
	//LogJson(tenantRes)
	if errtenantRes != nil {
...
func GetBuildLastTag(ctx context.Context, team, docker, tipo, tenant, accessToken, loginApiDomain, coreApiVersion, devopsToken string) (string, error) {
	...
	restyKubeImicroservRes, errKubeImicroservRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsImicro, "msdevops", "/api/"+os.Getenv("API_VERSION")+"/devops/KUBEDKRBUILD", devopsToken, loginApiDomain, coreApiVersion)
	if errKubeImicroservRes != nil {
		Logga(ctx, os.Getenv("JsonLog"), errKubeImicroservRes.Error())
...
func GetCurrentBranchSprint(ctx context.Context, team, tipo, tenant, accessToken, loginApiDomain, coreApiVersion, devopsToken string) (string, error) {
	...
	restyKubeImicroservRes, errKubeImicroservRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsImicro, "msdevops", "/api/"+os.Getenv("API_VERSION")+"/devops/KUBETEAMBRANCH", devopsToken, loginApiDomain, coreApiVersion)
	if errKubeImicroservRes != nil {
		return "", errKubeImicroservRes
	}
...
func GetEnvironmentStatus(ctx context.Context, cluster, enviro, microserice, customer, tenant, accessToken, loginApiDomain, coreApiVersion, resource string) error {
	...
	restyEsRes, errEsRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsEs, "msdevops", "/api/"+os.Getenv("API_VERSION")+"/devops/KUBEENVSTATUS", devopsToken, loginApiDomain, coreApiVersion)
	if errEsRes != nil {
...
func GetAccessCluster(ctx context.Context, cluster, devopsToken, loginApiDomain, coreApiVersion string, monolith bool) ClusterAccess {
	...
	restyKubeCluRes, _ := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsClu, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBECLUSTER", devopsToken, loginApiDomain, coreApiVersion)
...
func GetCustomerToken(ctx context.Context, accessToken, refappCustomer, resource, dominio, coreApiVersion string) (string, error) {
	...
	restyAuthResponse, restyAuthErr := ApiCallLOGIN(ctx, os.Getenv("RestyDebug"), argsAuth, "msauth", "/api/"+os.Getenv("API_VERSION")+"/auth/login", dominio, coreApiVersion)
	if restyAuthErr != nil {
		return "", restyAuthErr
	}
...
func GetCfToolEnv(ctx context.Context, token, dominio, tenant, coreApiVersion, enviro string, monolith bool) (TenantEnv, error) {
	...
	envRes, _ := ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "ms"+devops, "/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBECFTOOLENV", token, dominio, coreApiVersion)
...
func GetApiHostAndToken(ctx context.Context, enviro, cluster, token, apiDomain, coreApiVersion, swmono string) (string, string, error) {
	...
	if swmono == "mono" {
		endpointRes, errendpointRes = ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msdevopsmono", "/api/"+os.Getenv("API_VERSION")+"/devopsmono/KUBECLUSTER", token, apiDomain, coreApiVersion)
	} else {
		endpointRes, errendpointRes = ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msdevops", "/api/"+os.Getenv("API_VERSION")+"/devops/KUBECLUSTER", token, apiDomain, coreApiVersion)
	}
