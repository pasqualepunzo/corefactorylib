...
func GetUserGroup(ctx context.Context, token, gruppo, dominio, coreApiVersion string) (map[string]string, error) {
	...
	gruRes, _ := ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msusers", "/"+os.Getenv("CORE_API_ENVIRONMENT")+"-core/api/"+os.Getenv("API_VERSION")+"/users/GRU/equals(XGRU03,'"+gruppo+"')", token, dominio, coreApiVersion)
...
func GetNextVersion(ctx context.Context, masterBranch, nomeDocker, tenant, accessToken, loginApiDomain, coreApiVersion, resource string) (string, error) {
	...
	restyKubeImicroservRes, errImicroservRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsImicro, "msdevops", "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devops/KUBEDKRBUILD", devopsToken, loginApiDomain, coreApiVersion)
...
func GetMicroserviceDetail(ctx context.Context, team, ims, gitDevMaster, buildVersion, devopsToken, autopilot, enviro, dominio, coreApiVersion string) (Microservice, error) {
	...
	restyKubeImicroservRes, errKubeImicroservRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsImicro, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEIMICROSERV", devopsToken, dominio, coreApiVersion)
	...
	restyKubeCluRes, errKubeCluRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsClu, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBECLUSTER", devopsToken, dominio, coreApiVersion)
	...
	restyKubeMSRes, errKubeMSRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsMS, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEMICROSERV", devopsToken, dominio, coreApiVersion)
	if errKubeMSRes != nil {
	...
	if autopilot != "1" {
		...
		restyKubeHpaRes, errKubeHpaRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsHpa, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEMICROSERVHPA", devopsToken, dominio, coreApiVersion)
	...
	restyDkrLstRes, errDkrLstRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsDkr, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devops/custom/SELKUBEDKRLIST/values", devopsToken, dominio, coreApiVersion)
	if errDkrLstRes != nil {
	...
	if len(restyDkrLstRes.BodyArray) > 0 {
		...
		for _, x := range restyDkrLstRes.BodyArray {
			...
			restyKubeBldRes, errKubeBldRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsBld, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/custom/KUBEDKRBUILD/values", devopsToken, dominio, coreApiVersion)
			...
			restyKubeMntRes, errKubeMntRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsMnt, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEDKRMOUNT", devopsToken, dominio, coreApiVersion)
			...
			restyKubeSrcRes, errKubeSrcRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsSrc, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEDKRRESOURCE", devopsToken, dominio, coreApiVersion)
			...
			restyKubePrbRes, errKubePrbRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsProbes, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBEDKRPROBE", devopsToken, dominio, coreApiVersion)
...
func GetTenant(ctx context.Context, token, dominio, coreApiVersion string) ([]Tenant, error) {
	...
	tenantRes, errtenantRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msauth", "/"+os.Getenv("CORE_API_ENVIRONMENT")+"-core/api/"+os.Getenv("API_VERSION")+"/auth/tenants", token, dominio, coreApiVersion)
	//LogJson(tenantRes)
	if errtenantRes != nil {
...
func GetProfileInfo(ctx context.Context, token, dominio, coreApiVersion string) (map[string]interface{}, error) {
	...
	args := make(map[string]string)
	infoRes, _ := ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msauth", "/"+os.Getenv("CORE_API_ENVIRONMENT")+"-core/api/"+os.Getenv("API_VERSION")+"/auth/getProfileInfo", token, dominio, coreApiVersion)
...
func GetBuildLastTag(ctx context.Context, team, docker, tipo, tenant, accessToken, loginApiDomain, coreApiVersion, devopsToken string) (string, error) {
	...
	restyKubeImicroservRes, errKubeImicroservRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsImicro, "msdevops", "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devops/KUBEDKRBUILD", devopsToken, loginApiDomain, coreApiVersion)
	if errKubeImicroservRes != nil {
		Logga(ctx, os.Getenv("JsonLog"), errKubeImicroservRes.Error())
...
func GetCurrentBranchSprint(ctx context.Context, team, tipo, tenant, accessToken, loginApiDomain, coreApiVersion, devopsToken string) (string, error) {
	...
	restyKubeImicroservRes, errKubeImicroservRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsImicro, "msdevops", "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devops/KUBETEAMBRANCH", devopsToken, loginApiDomain, coreApiVersion)
	if errKubeImicroservRes != nil {
		return "", errKubeImicroservRes
	}
...
func GetEnvironmentStatus(ctx context.Context, cluster, enviro, microserice, customer, tenant, accessToken, loginApiDomain, coreApiVersion, resource string) error {
	...
	restyEsRes, errEsRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsEs, "msdevops", "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devops/KUBEENVSTATUS", devopsToken, loginApiDomain, coreApiVersion)
	if errEsRes != nil {
...
func SetEnvironmentStatus(ctx context.Context, cluster, enviro, microserice, customer, user, toggle, tenant, accessToken, loginApiDomain, coreApiVersion, resource string) error {
	...
	res := ApiCallPOST(ctx, os.Getenv("RestyDebug"), keyvalueslices, "msdevops", "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devops/KUBEENVSTATUS", devopsToken, loginApiDomain, coreApiVersion)
...
func GetAccessCluster(ctx context.Context, cluster, devopsToken, loginApiDomain, coreApiVersion string, monolith bool) ClusterAccess {
	...
	restyKubeCluRes, _ := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsClu, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBECLUSTER", devopsToken, loginApiDomain, coreApiVersion)
...
func GetCustomerToken(ctx context.Context, accessToken, refappCustomer, resource, dominio, coreApiVersion string) (string, error) {
	...
	restyAuthResponse, restyAuthErr := ApiCallLOGIN(ctx, os.Getenv("RestyDebug"), argsAuth, "msauth", "/"+os.Getenv("CORE_API_ENVIRONMENT")+"-core/api/"+os.Getenv("API_VERSION")+"/auth/login", dominio, coreApiVersion)
	if restyAuthErr != nil {
		return "", restyAuthErr
	}
...
func GetCfToolEnv(ctx context.Context, token, dominio, tenant, coreApiVersion, enviro string, monolith bool) (TenantEnv, error) {
	...
	envRes, _ := ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBECFTOOLENV", token, dominio, coreApiVersion)
...
func DeleteObsoleteObjects(ctx context.Context, ires IstanzaMicro, versione, canaryProduction, namespace, enviro, tenant, devopsToken, dominio, coreApiVersion string, debug string) error {
	...
	restyDeployRes, errDeployRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsDeploy, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/"+devops+"/DEPLOYLOG", devopsToken, dominio, coreApiVersion)
	if errDeployRes != nil {
...
func GetOverrideTenantEnv(ctx context.Context, bearerToken, team string, tntEnv TenantEnv, dominio, coreApiVersion string) (TenantEnv, error) {
	...
	envRes, _ := ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msdevops", "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devops/KUBETEAMBRANCH", bearerToken, dominio, coreApiVersion)
...
func GetApiHostAndToken(ctx context.Context, enviro, cluster, token, apiDomain, coreApiVersion, swmono string) (string, string, error) {
	...
	if swmono == "mono" {
		endpointRes, errendpointRes = ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msdevopsmono", "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devopsmono/KUBECLUSTER", token, apiDomain, coreApiVersion)
	} else {
		endpointRes, errendpointRes = ApiCallGET(ctx, os.Getenv("RestyDebug"), args, "msdevops", "/"+os.Getenv("APP_ENV")+"-devops/api/"+os.Getenv("API_VERSION")+"/devops/KUBECLUSTER", token, apiDomain, coreApiVersion)
	}
