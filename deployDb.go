	...
	*/
	...
	if len(restyKubeCluRes.BodyJson) > 0 {
		...
		**/
		...
		restyKubeCluEnvRes, errKubeCluEnvRes := ApiCallGET(ctx, os.Getenv("RestyDebug"), argsCluEnv, "ms"+devops, "/"+os.Getenv("APP_ENV")+"-devops/"+os.Getenv("API_VERSION")+"/"+devops+"/KUBECLUSTERENV", devopsTokenDst, dominio, coreApiVersion)
